...
public class GeoScrapingService {
     ...
     */
    public ScrapedData extractDataForAi(String url, CategoryStatus categoryStatus) {
        try {
            log.info("[Geo Scraping] Jsoup 스크래핑 시작 - URL: {}", url);
            ScrapedData jsoupResult = scrapeByJsoup(url, categoryStatus);
            ...
            return scrapeByPlaywright(url, categoryStatus);
    ...
    private ScrapedData scrapeByJsoup(String url, CategoryStatus categoryStatus) throws IOException {
        Document doc = Jsoup.connect(url)
                .userAgent("Mozilla/5.0")
                .timeout(5000)
                .get();
        return buildScrapedData(url, categoryStatus, doc);
    }

    private ScrapedData scrapeByPlaywright(String url, CategoryStatus categoryStatus) {
        try (Playwright playwright = Playwright.create()) {
            Browser browser = playwright.chromium().launch(new BrowserType.LaunchOptions().setHeadless(true));
            Page page = browser.newPage();
            page.navigate(url, new Page.NavigateOptions().setWaitUntil(WaitUntilState.DOMCONTENTLOADED).setTimeout(15000));
    ...
    private ScrapedData buildScrapedData(String url, CategoryStatus categoryStatus, Document doc) {
        ...
        return new ScrapedData(url, categoryStatus.toString(), cleanHtmlText, combinedJsonLd);
    }
    ...
    //AI 서버의 토큰량 한계(8192)로 인해 본문 텍스트는 8190자로 제한. (그냥 앞에서 부터 자름.)
    //GeoAiService에서 AI 서버에게 전송할때 8190자로 제한하는데 로컬 환경에서도 동일하게 하려고 하는 메서드
    private String limitTextLength(String text){
        final int maxLength = 8190;
