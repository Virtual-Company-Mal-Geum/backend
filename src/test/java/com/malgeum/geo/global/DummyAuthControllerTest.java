...
public class DummyAuthControllerTest {
    ...
    void dummy_fake_login() throws Exception {
        //given: 가짜 JwtTokenProvider가 "999"라는 아이디를 받으면 "fake-jwt-token"을 주도록 조작(Stubbing)합니다.
        given(jwtTokenProvider.generateToken("999", List.of("ROLE_USER"))).willReturn("fake-jwt-token");
    ...
    void accessSecuredApiWithValidToken_ShouldSucceed() throws Exception {
        ...
        // 토큰에서 아이디를 꺼낼 때는 "999"를 반환하도록 조작합니다.
        given(jwtTokenProvider.getAuthentication(validToken))
               .willReturn(new UsernamePasswordAuthenticationToken("999", "", List.of()));

        // when & then: 헤더에 토큰을 싣고 요청을 쏩니다!
        mockMvc.perform(get("/api/v1/auth/secured")
               .header("Authorization", "Bearer " + validToken))
               .andExpect(status().isOk())
               .andExpect(content().string("JWT 필터 통과 성공! 환영합니다, 고객사 ID: 999"));
    }
}
